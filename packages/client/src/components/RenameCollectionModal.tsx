 ...
 * 
 * Features:
 * - Auto-focus input on open with current name pre-filled
 * - Enter key to submit
 * - Escape key to close
 * - Disabled rename button when empty
 * - Clears error after successful submit
 * - Displays validation errors
 * - Closes modal after successful rename
 */
export function RenameCollectionModal({ isOpen, currentName, onClose, onSubmit }: RenameCollectionModalProps) {
  const [name, setName] = useState('');
  const [error, setError] = useState('');
  const inputRef = useRef<HTMLInputElement>(null);

  // Initialize name with currentName when modal opens
  useEffect(() => {
    if (isOpen) {
      setName(currentName);
  ...
  const isRenameDisabled = name.trim().length === 0;
