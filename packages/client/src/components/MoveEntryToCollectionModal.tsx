...
interface MoveEntryToCollectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  entry: Entry | null;
 ...
 * 
 * Allows user to migrate an entry to a different collection.
 * 
 * Features:
 * - Shows all collections except current
 * - Includes "Uncategorized" option (null collection)
 * - Calls appropriate migrate handler based on entry type
 * - Closes on success or cancel
 * - Prevents migration if entry already migrated
 */
export function MoveEntryToCollectionModal({
  isOpen,
  onClose,
  entry,
  currentCollectionId,
  collections,
  onMigrate,
}: MoveEntryToCollectionModalProps) {
  ...
  const handleMigrate = async (targetCollectionId: string | null) => {
    ...
    >
      ...
      >
        <h2 id="modal-title" className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
          Move {getEntryTypeLabel()} to collection
        </h2>
        ...
        <div className="space-y-2 mb-6">
          ...
          {currentCollectionId !== undefined && (
            <button
              ...
              className="w-full text-left px-4 py-3 rounded border border-gray-200 dark:border-gray-700
          ...
          {availableCollections.map(collection => (
            <button
              key={collection.id}
              onClick={() => handleMigrate(collection.id)}
              disabled={isSubmitting || isAlreadyMigrated}
              className="w-full text-left px-4 py-3 rounded border border-gray-200 dark:border-gray-700
                         hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors
                         disabled:opacity-50 disabled:cursor-not-allowed"
